...
function createRemoteServer(cloudAgentInstallerServerURL) {
	...
	var downloadFile = (function() {
		...
		var componentPathAPIUrl = cloudAgentInstallerServerURL + "/getFile/" + util.installerOS + "/";

		function getComponentPath(file, version) {
			var fileComponents = file.split("/");
			fileComponents.push(encodeURIComponent(fileComponents.pop()));
			return componentPathAPIUrl + fileComponents.join("/") + "/" + version;
		}
		...
		return function(file, version) {

			var url = getComponentPath(file, version);
	...
	function osTypeData() {
		var path = "os=" + util.installerOS;
		if (undefined !== util.osBitSize) {
	...
	return {
		...
		getFilesByCategory: function getFilesByCategory(categories, version) {

			var path = cloudAgentInstallerServerURL + "/getFilesByCategory?" + osTypeData();
			if (categories) {
		...
		generateDesktopDB: function generateDesktopDB(version) {

			var url = cloudAgentInstallerServerURL + "/generateDesktopDB?";
			url += "os=" + util.installerOS;
			if (version) {
				url += "&version=" + version;
			}
