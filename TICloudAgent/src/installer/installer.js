...
var createInstaller = function(userDataRoot, remoteServer, db, options) {
	...
	var installFiles = function(fetchInstallInfo) {
		...
		fetchInstallInfo()
			...
			.then(function(filesMissing) {
				...
				return remoteServer.downloadFiles(filesMissing)
	...
	return {
		...
		generateDesktopInstall: function(categories, version) {
			return remoteServer.generateDesktopDB(version)
				.then(function() {
					return installFiles(function() {
						return remoteServer.getFilesByCategory(categories, version);
					});
				});
		},
